(function($) {
    ...
    $.fn.orgChart.defaults = {
        data: [{id:1, name:'Root', parent: 0, org_id:0}],
        showControls: false,
        allowEdit: false,
        onAddNode: null,
        onDeleteNode: null,
    ...
    function OrgChart($container, opts){
        ...
        this.draw = function(){
            ...
            $container.find('.node button').click(function(){
        ...
        this.updateNode = function(id,htmlcode=null,jabatan=null){
         
            if(htmlcode!==null) { 
                nodes[id].data.description=htmlcode;
                nodes[id].data.name=jabatan
                $container.find('div[node-id='+id+'] .description').html(htmlcode); 
                $container.find('div[node-id='+id+'] h2').html(jabatan);
                self.draw();
            }
            
        }

        this.newNode = function(parentId,org_id,newId=null,htmlcode=null){
            if(newId!==null){
                var nextId = newId;
            }else{
            ...
            let obj = {id: nextId, name: 'Unknown', parent: parentId,org_id:org_id}
            
            if(htmlcode!==null)obj.description=htmlcode
    ...
    function Node(data){
        ...
        this.formatNode = function(opts){
            ...
            if(typeof data.description !== 'undefined'){
                descString = '<div class="col-12 description">'+self.data.description+'</div>';
            }
            if(opts.showControls){
                var buttonsHtml = "<div class='org-add-button'>"+opts.newNodeText+"</div>";
                if(data.parent) buttonsHtml+="<div class='org-del-button'></div>";
            }
            else{
                buttonsHtml = '';
            }
            return "<div class='node card' node-id='"+this.data.id+"'>"+nameString+descString+buttonsHtml+"</div>";
        }
    }
