        ...
        <div class="content-wrapper">
            <div class="row breadcrumbs-top">
                <div class="col-12 mb-1">
                    <a href="{{ url('faskes/tenaga-medis') }}"><button type="button" class="btn btn-outline-primary"><i class="mr-75" data-feather="arrow-left"></i>Kembali</button></a>
                </div>
                <div class="col-12">
                    @if (isset($dokter))
                        <h2 class="content-header-title float-left">Edit Data Tenaga Medis</h2>
                    @else
                        <h2 class="content-header-title float-left">Tambah Data Tenaga Medis</h2>
                    @endif
                </div>
            </div>
            <div class="content-body">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            @if (isset($dokter))
                                <form action="{{ route('faskes.tenaga-medis.update',$dokter->id_dokter) }}" class="default-form" autocomplete="off">
                                    @method('PUT')
                            @else
                                <form action="{{ url('faskes/tenaga-medis') }}" class="default-form" autocomplete="off">
                            @endif
                                @csrf
                                <div class="card-body">
                                    ...
                                    <div class="row">
                                            ...
                                            <select class="select2 form-control form-control-lg" name="id_kategori_dokter" id="id_kategori_dokter">
                                                ...
                                                @foreach ($kategori_dokter as $kd)
                                                    <option value="{{ $kd->id_kategori_dokter }}"
                                                        @isset($dokter)
                                                            {{ ($dokter->jenis_spesialis->id_kategori_dokter == $kd->id_kategori_dokter)?'selected':''}}
                                                        @endisset
                                        ...
                                        <div class="col-md-6 col-12">             
                                            <div class="form-group form-input">
                                                ...
                                                <select name="id_spesialis" id="id_spesialis" class="select2 form-control"></select>
...
@section('page_script')
    <script>
        ...
        @if(isset($dokter))
            ...
            select_jenis_spesialis({{ $dokter->jenis_spesialis->id_kategori_dokter }})
            setTimeout(function(){$("#id_spesialis").val("{{ $dokter->id_spesialis }}").trigger('change.select2')}, 1000);
            if ({{ $dokter->klasifikasi }} === 'pns') {
                $('#matra').prop('disabled',true)
                $('#pangkat_korps').prop('disabled',true)
        ...
        $('input[name="klasifikasi"]').change(function(){
            ...
            if (klasifikasi === 'pns') {
                ...
                $('#pangkat_korps').prop('disabled',false)
