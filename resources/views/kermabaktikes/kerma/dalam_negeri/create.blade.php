...
@section('main')
    ...
    <div class="app-content content ">
        ...
        <div class="content-wrapper">
            <div class="row pb-1">
                <div class="col-6">
                    <a href="{{ url('kerma/kdn') }}"><button type="button" class="btn btn-outline-primary">
            ...
            <div class="row pb-1">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ request()->segment(3)=='create'?'Input':'Edit' }} Data Kerma</h2>
                </div>
            </div>
            <div class="content-body">
                ...
                <section id="multilingual-datatable">
                    @if (isset($kdn))
                        <form action="{{ route('kerma.kdn.update',$kdn->id_kerma) }}" class="default-form" autocomplete="off">
                            @method('PUT')
                    @else
                        <form action="{{ route('kerma.kdn.store') }}" class="default-form" autocomplete="off">
                    @endif
                        @csrf
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                            ...
                                            <select class="select2 form-control form-control-lg" name="id_kegiatan">
                                                ...
                                                @foreach ($kegiatan as $item)
                                                    <option value="{{ $item->id_kegiatan }}"
                                                        @isset($kdn)
                                                            {{ ($kdn->id_kegiatan == $item->id_kegiatan)?'selected':'' }}
                                                        @endisset
                                        ...
                                        <div class="form-group form-input mt-1">
                                            ...
                                            <input type="text" name="nama_acara" id="nama_acara" class="form-control" placeholder="Nama Acara" value="{{ $kdn->nama_acara??null }}">
                                            <div class="invalid-feedback"></div>
                                        </div>
                                        <div class="form-group form-input">
                                            ...
                                            <select class="select2 form-control form-control-lg" name="id_jenis_keg">
                                                ...
                                                @foreach ($jenis_kegiatan as $item)
                                                    <option value="{{ $item->id_jenis_keg }}"
                                                        @isset($kdn)
                                                            {{ ($kdn->id_jenis_keg == $item->id_jenis_keg)?'selected':'' }}
                                                        @endisset
                                        ...
                                        <div class="form-group form-input">
                                            ...
                                            <input type="text" id="tgl_pelaksanaan" class="form-control flatpickr-basic" placeholder="Tanggal Kegiatan" name="tgl_pelaksanaan" value="{{ $kdn->tgl_pelaksanaan??null }}"/>
                                            <div class="invalid-feedback"></div>
                                        </div>
                                        <div class="form-group form-input">
                                            <label class="form-label" for="tempat">Tempat</label>
                                            <input type="text" id="tempat" class="form-control" placeholder="Tempat" name="tempat" value="{{ $kdn->tempat??null }}"/>
                                            <div class="invalid-feedback"></div>
                                        </div>
                                            ...
                                            @foreach ($keterangan as $item)
                                                <div class="custom-control custom-radio mt-0">
                                                    <input type="radio" id="{{ $item->id_keterangan }}" name="id_keterangan" class="custom-control-input" value="{{ $item->id_keterangan }}"
                                                        @isset($kdn)
                                                            {{ ($kdn->id_keterangan == $item->id_keterangan)?'checked':'' }}
                                                        @endisset
                                                        />
                                        ...
                                        <div class="demo-inline-spacing">
                                            @foreach ($status as $item)
                                                <div class="custom-control custom-radio mt-0">
                                                    <input type="radio" id="status_{{ $item->id_status }}" name="id_status" class="custom-control-input" value="{{ $item->id_status }}"
                                                        @isset($kdn)
                                                            {{ ($kdn->id_status == $item->id_status)?'checked':'' }}
                                                        @endisset
                                                        />
                                                    <label class="custom-control-label" for="status_{{ $item->id_status }}">{{ $item->nama_status }}</label>
                                                </div>  
                                            @endforeach
                                            <div class="invalid-feedback"></div>
                                        </div>
