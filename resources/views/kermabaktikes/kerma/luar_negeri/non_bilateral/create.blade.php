...
@section('main')
    ...
    <div class="app-content content ">
        ...
        <div class="content-wrapper">
            <div class="row pb-1">
                <div class="col-6">
                    <a href="{{ url('kerma/nonbilateral') }}"><button type="button" class="btn btn-outline-primary">
            ...
            <div class="row pb-1">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">@if (isset($nonbilateral))
                        Edit Data Multilateral
                    @else
                        Input Data Multilateral
                    @endif </h2>
                </div>
            </div>
            <div class="content-body">
                ...
                <section id="multilingual-datatable">
                    @if (isset($nonbilateral))
                        <form action="{{ route('kerma.nonbilateral.update',$nonbilateral->id_kerma) }}" class="default-form" autocomplete="off">
                            @method('PUT')
                    @else
                        <form action="{{ route('kerma.nonbilateral.store') }}" class="default-form" autocomplete="off">
                    @endif
                        @csrf
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        <div class="form-group form-input1">
                                            <label class="form-label" for="nama">Nama Kegiatan</label>
                                            <select class="select2 form-control form-control-lg" name="id_kegiatan">
                                                <option selected disabled>Nama Kegiatan</option>
                                                @foreach ($kegiatan as $item)
                                                    <option value="{{ $item->id_kegiatan }}"
                                                        @isset($nonbilateral)
                                                            {{ ($nonbilateral->id_kegiatan == $item->id_kegiatan)?'selected':'' }}
                                                        @endisset
                                        ...
                                        <div class="form-group form-input mt-1">
                                            ...
                                            <input type="text" name="nama_acara" id="nama_acara" class="form-control" placeholder="Nama Acara" value="{{ $nonbilateral->nama_acara??null }}">
                                            <div class="invalid-feedback"></div>
                                        </div>
                                            ...
                                            <select class="select2 form-control form-control-lg" name="id_jenis_keg">
                                                ...
                                                @foreach ($jenis_kegiatan as $item)
                                                    <option value="{{ $item->id_jenis_keg }}"
                                                        @isset($nonbilateral)
                                                            {{ ($nonbilateral->id_jenis_keg == $item->id_jenis_keg)?'selected':'' }}
                                                        @endisset
                                        ...
                                        <div class="form-group form-input">
                                            ...
                                            <input type="text" id="tgl_pelaksanaan" class="form-control flatpickr-basic" placeholder="Tanggal Kegiatan" name="tgl_pelaksanaan" value="{{ $nonbilateral->tgl_pelaksanaan??null }}"/>
                                            <div class="invalid-feedback"></div>
                                        </div>
                                        <div class="form-group form-input">
                                            <label class="form-label" for="tempat">Tempat</label>
                                            <input type="text" id="tempat" class="form-control" placeholder="Tempat" name="tempat" value="{{ $nonbilateral->tempat??null }}"/>
                                            <div class="invalid-feedback"></div>
                                        </div>
                                            ...
                                            @foreach ($keterangan as $item)
                                                <div class="custom-control custom-radio mt-0">
                                                    <input type="radio" id="{{ $item->id_keterangan }}-{{ $item->keterangan }}" name="id_keterangan" class="custom-control-input" value="{{ $item->id_keterangan }}"
                                                        @isset($nonbilateral)
                                                            {{ ($nonbilateral->id_keterangan == $item->id_keterangan)?'checked':'' }}
                                                        @endisset
                                                        />
                                            ...
                                            @foreach ($status as $item)
                                                <div class="custom-control custom-radio mt-0">
                                                    <input type="radio" id="{{ $item->id_status }}-{{ $item->nama_status }}" name="id_status" class="custom-control-input" value="{{ $item->id_status }}"
                                                        @isset($nonbilateral)
                                                            {{ ($nonbilateral->id_status == $item->id_status)?'checked':'' }}
                                                        @endisset
                                                        />
...
@section('page_script')
    ...
    <script>
        ...
        $(function(){
            $("#periode").val({{ $nonbilateral->periode??null }})
        })
