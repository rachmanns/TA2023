...
@section('main')
    ...
    <div class="app-content content ">
        ...
        <div class="content-wrapper">
            <div class="row pb-1">
                <div class="col-6">
                    <a href="{{ url('kerma/bilateral') }}"><button type="button" class="btn btn-outline-primary">
            ...
            <div class="content-body">
                ...
                <section id="multilingual-datatable">
                    @if (isset($bilateral))
                        <form action="{{ route('kerma.bilateral.update',$bilateral->id_kerma) }}" class="default-form" autocomplete="off">
                            @method('PUT')
                    @else
                        <form action="{{ route('kerma.bilateral.store') }}" class="default-form" autocomplete="off">
                    @endif
                        @csrf
                        <div class="row">
                            <div class="col-12">
                                    ...
                                    <div class="card-body">  
                                        <div class="demo-inline-spacing">                         
                                            <div class="custom-control custom-radio mt-0">
                                                <input type="radio" id="usibdd" name="event" class="custom-control-input" value="1"
                                                    @isset($bilateral)
                                                        {{ ($bilateral->kegiatan->id_event==1)?'checked':'' }}
                                                    @endisset
                                                />
                                                <label class="custom-control-label" for="usibdd">USIBDD</label>
                                            </div>   
                                            <div class="custom-control custom-radio mt-0">
                                                <input type="radio" id="thainesia" name="event" class="custom-control-input" value="2"
                                                    @isset($bilateral)
                                                        {{ ($bilateral->kegiatan->id_event==2)?'checked':'' }}
                                                    @endisset
                                                />
                                                <label class="custom-control-label" for="thainesia">THAINESIA</label>
                                            </div>   
                                        </div>
                                        <div class="form-group form-input mt-1">
                                            <label class="form-label" for="nama">Nama Kegiatan</label>
                                            <select class="select2 form-control form-control-lg" name="id_kegiatan" id="id_kegiatan">
                                            </select>
                                            <div class="invalid-feedback"></div>
                                        </div>
                                        <div class="form-group form-input mt-1">
                                            <label class="form-label" for="nama">Nama Acara</label>
                                            <input type="text" name="nama_acara" id="nama_acara" class="form-control" placeholder="Nama Acara" value="{{ $bilateral->nama_acara??null }}">
                                            <div class="invalid-feedback"></div>
                                        </div>
                                        <div class="form-group form-input">
                                            <label class="form-label text-muted" for="jenis">Jenis Kegiatan</label>
                                            <select class="select2 form-control form-control-lg" name="id_jenis_keg">
                                                <option selected disabled>Jenis Kegiatan</option>
                                                @foreach ($jenis_kegiatan as $item)
                                                    <option value="{{ $item->id_jenis_keg }}"
                                                        @isset($bilateral)
                                                            {{ ($bilateral->id_jenis_keg == $item->id_jenis_keg)?'selected':'' }}
                                                        @endisset 
                                                        >{{ $item->jenis_keg }}</option>
                                                @endforeach
                                            </select>
                                            <div class="invalid-feedback"></div>
                                        </div>
                                        <div class="form-group form-input">
                                            ...
                                            <input type="text" id="tgl" class="form-control flatpickr-basic" placeholder="Tanggal Kegiatan"  name="tgl_pelaksanaan" value="{{ $bilateral->tgl_pelaksanaan??null }}"/>
                                            <div class="invalid-feedback"></div>
                                        </div>
                                        <div class="form-group form-input">
                                            <label class="form-label" for="tempat">Tempat</label>
                                            <input type="text" id="tempat" class="form-control" placeholder="Tempat" name="tempat" value="{{ $bilateral->tempat??null }}"/>
                                            <div class="invalid-feedback"></div>
                                        </div>
                                        <label class="form-label" for="periode">Tahun</label>
                                        <div class="input-group input-group-merge form-input">
                                            <input type="text" id="periode" class="yearpicker form-control bg-white cursor-pointer" placeholder="Tahun" name="periode"/>
                                        ...
                                        <div class="demo-inline-spacing">
                                            @foreach ($keterangan as $item)
                                                <div class="custom-control custom-radio mt-0">
                                                    <input type="radio" id="{{ $item->id_keterangan }}-{{ $item->keterangan }}" name="id_keterangan" class="custom-control-input" value="{{ $item->id_keterangan }}" 
                                                        @isset($bilateral)
                                                            {{ ($bilateral->id_keterangan == $item->id_keterangan)?'checked':'' }}
                                                        @endisset 
                                                        />
                                                    <label class="custom-control-label" for="{{ $item->id_keterangan }}-{{ $item->keterangan }}">{{ $item->keterangan }}</label>
                                                </div>   
                                            @endforeach
                                        </div>
                                        ...
                                        <div class="demo-inline-spacing">
                                            @foreach ($status as $item)
                                                <div class="custom-control custom-radio mt-0">
                                                    <input type="radio" id="{{ $item->id_status }}-{{ $item->nama_status }}" name="id_status" class="custom-control-input" value="{{ $item->id_status }}"
                                                        @isset($bilateral)
                                                            {{ ($bilateral->id_status == $item->id_status)?'checked':'' }}
                                                        @endisset 
                                                        />
                                                    <label class="custom-control-label" for="{{ $item->id_status }}-{{ $item->nama_status }}">{{ $item->nama_status }}</label>
                                                </div>   
                                            @endforeach
                                        </div>
                                        <div class="form-group form-input mt-1">
...
@section('page_script')
    ...
    <script>
        ...
        $(function(){
            $("#periode").val({{ $bilateral->periode??null }})
            select_kegiatan({{ $bilateral->kegiatan->id_event??null }})
            setTimeout(function(){$("#id_kegiatan").val("{{ $bilateral->id_kegiatan??null }}").trigger('change.select2')}, 1000);
        ...
        $(document).on('change', 'input[name="event"]', function() {
            var id_event = $(this).val();
            select_kegiatan(id_event)
        });

        function select_kegiatan(id_event){
            $.ajax({
                url: "{{ url('kerma/bilateral/get-kegiatan') }}"+"/"+id_event, 
                method: "GET",
                dataType: "json",
                success: function (result) {
                    
                    $("#id_kegiatan").empty().trigger("change");
                    
                    $("#id_kegiatan").select2({ data: result.data,placeholder: "Pilih Kegiatan",allowClear: true });
