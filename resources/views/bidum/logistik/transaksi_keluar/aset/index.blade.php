            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-1">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-left mb-0">Daftar Transaksi Keluar - Aset <font id="tgl_title"></font></h2>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div class="card-datatable">
                                    <nav class="nav-justified">
                                        <div class="nav nav-tabs mb-0" id="nav-tab" role="tablist">
                                            <a class="nav-item nav-link active m-2 mb-0" id="nav-tk-tab" data-toggle="tab"
                                                href="#nav-tk" role="tab" aria-controls="nav-tk"
                                                aria-selected="true"><span class="font-medium-4 font-weight-bolder">Transfer
                                                    Keluar</span></a>
                                    ...
                                    <div class="tab-content" id="nav-tabContent">
                                        <div class="tab-pane fade show active" id="nav-tk" role="tabpanel"
                                            ...
                                            @include('bidum.logistik.transaksi_keluar.aset.form_transfer')
                                        </div>
                                        <div class="tab-pane fade" id="nav-hibah" role="tabpanel"
                                            ...
                                            @include('bidum.logistik.transaksi_keluar.aset.form_hibah')
                                        </div>
                                    </div>
                                </div>
                            </div>
...
@section('page_script')
    <script>
        ...
        function report_list(from_date, to_date) {
            ...
            let url_transfer = `{{ url('bidum/logistik/transfer-keluar/list/aset') }}/${from_date}/${to_date}`;
            let url_hibah = `{{ url('bidum/logistik/hibah-keluar/list/A') }}/${from_date}/${to_date}`;
        ...
        function edit_transfer(e) {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('bidum/logistik/transfer-keluar/edit') }}" + '/' + id_out_pemakaian,
                success: function(response) {
                    $('#transfer_modal form').attr('action', action);
                    $('#transfer_modal').find('#no_nota_dinas').html(response.rencana_pengeluaran == null ? '' :
                        response.rencana_pengeluaran.no_nota_dinas);
        ...
        function edit_hibah(e) {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('bidum/logistik/hibah-keluar/edit') }}" + '/' + id_out_pemakaian,
                success: function(response) {
                    $('#hibah_modal form').attr('action', action);
                    $('#hibah_modal').find('#no_nota_dinas').html(response.rencana_pengeluaran == null ? '' : response.rencana_pengeluaran.no_nota_dinas);
