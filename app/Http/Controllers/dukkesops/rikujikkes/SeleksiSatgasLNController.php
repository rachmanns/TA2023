...
class SeleksiSatgasLNController extends Controller
{
    ...
    public function create($keterangan)
    {
        ...
        $kategori_duk = PenugasanSatgas::select('id_tugas', 'nama_satgas', 'nama_batalyon', 'tahun_anggaran')->get();

        return view('dukkesops.rikujikkes.seleksi_satgas_ln.create', compact('active_menu', 'kategori_duk', 'keterangan'));
    }
    ...
    public function preview(KegiatanDukRequest $request)
    {
        ...
        $kegiatan_duk['jenis_kegiatan'] = 'satgas ln';
    ...
    {
        $kegiatan_duk = KegiatanDuk::with('penugasan_satgas')->where('jenis_kegiatan', KegiatanDuk::SATGAS_LN)->where('keterangan', $request->keterangan)
        ...
        return DataTables::of($kegiatan_duk)
            ...
            ->addColumn('action', function ($row) {
                ...
                return "<div class='text-center'><a href='" . url('dukkesops/seleksi-satgas-ln/' . $row->id_kegiatan_duk . '/show') . "'><button title='Detail' class='btn text-primary p-0 pr-50'><i data-feather='file-text' class='font-medium-4'></i></button></a><button title='Delete' type='button' class='delete-data btn p-0' data-id='" . $row->id_kegiatan_duk . "' data-url='" . url('dukkesops/seleksi-satgas-ln') . "'><i data-feather='trash' class='font-medium-4 text-danger'></i></button></div>";
            })
    ...
    public function show(KegiatanDuk $seleksi_satgas_ln)
    {
        ...
        $chart = [
            'hasil_um' => $data_kegiatan_duk->groupBy('hasil_um')->map->count(),
            'hasil_wa' => $data_kegiatan_duk->groupBy('hasil_wa')->map->count(),
            'um_wa' => $data_kegiatan_duk->pluck('hasil_um')->merge($data_kegiatan_duk->pluck('hasil_wa'))->countby()
        ];
        return view('dukkesops.rikujikkes.seleksi_satgas_ln.hasil_rikkes', compact('active_menu', 'seleksi_satgas_ln', 'chart'));
    }
